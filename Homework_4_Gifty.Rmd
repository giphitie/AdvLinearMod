...
```{r simulation_code, message=FALSE, warning=FALSE}
...
if (pv<FWER) results[i,1] = 1
# 2. Bonferroni adjustment
betahat = coef(fullmodel)
s = summary(fullmodel)$sigma
Sigma = summary(fullmodel)$cov.unscaled # (X’X)^{-1}
t1 = abs(t(c1)%*%betahat)/(s*sqrt(t(c1)%*%Sigma%*%c1))
pval1 = 2*(1-pt(t1,df=n-5))
t2 = abs(t(c2)%*%betahat)/(s*sqrt(t(c2)%*%Sigma%*%c2))
pval2 = 2*(1-pt(t2,df=n-5))
t3 = abs(t(c3)%*%betahat)/(s*sqrt(t(c3)%*%Sigma%*%c3))
pval3 = 2*(1-pt(t3,df=n-5))
